...
private[spark] class UnifiedMemoryManager(
  ...
  extends MemoryManager(
    conf,
    numCores,
    onHeapStorageRegionSize, // 存储
    maxHeapMemory - onHeapStorageRegionSize) {  // 执行
   ...
   *
   * 在某些情况下，此方法会阻塞，直到有足够的内存，以确保每一个task在溢写到磁盘之前有机会上升到总内存池的1/2*活着的task数量，
   * 比如：当有新的task产生，但是老的task拥有太多的内存的时候就会这样
   */
...
object UnifiedMemoryManager {
  ...
  def apply(conf: SparkConf, numCores: Int): UnifiedMemoryManager = {
    ...
    new UnifiedMemoryManager(
      ...
      onHeapStorageRegionSize =
        (maxMemory * conf.get(config.MEMORY_STORAGE_FRACTION)).toLong,  // 存储内存 = maxMemory * spark.memory.storageFraction(0.5)
      numCores = numCores)
  }
