...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    // 通知SparkContext，TaskScheduler已经设置好了
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    // 每个executor的资源至少需要满足一个task需要的资源
    def checkResourcesPerTask(clusterMode: Boolean, executorCores: Option[Int]): Unit = {
      val taskCores = sc.conf.get(CPUS_PER_TASK) // 一个task至少需要1个core
      val execCores = if (clusterMode) { // executor拥有的core的数量
      ...
      // 有的clustermanager不支持EXECUTOR_CORES这个参数，
