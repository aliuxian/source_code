...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    val mapOutputTracker = if (isDriver) {
      ...
      // executor端 向driver端获取map输出信息
      new MapOutputTrackerWorker(conf)
    }

    // Have to assign trackerEndpoint after initialization as MapOutputTrackerEndpoint requires the MapOutputTracker itself
    // 注册endpoint
    ...
    val shuffleMgrName = conf.get(config.SHUFFLE_MANAGER)  // 默认是sort
    val shuffleMgrClass =
    ...
    // 内存管理
    // 3.0 开始没有静态内存管理了
    // 1.6开始默认是默认是统一管理，可指定为静态通过参数：spark.memory.useLegacyMode
