...
private[spark] class ApplicationMaster(
  ...
  def runUnmanaged(
    ...
    try {
      ...
      reporterThread.join() // 当前线程等待reporterThread线程执行结束
    } catch {
      case e: Exception =>
        // catch everything else if not specifically handled
        logError("Uncaught exception: ", e)
  ...
  private def createAllocator(
    ...
    // driverUrl = s"spark://$name@${rpcAddress.host}:${rpcAddress.port}"
    //  分布式缓存
