...
public class CreateTableProcedure
  ...
  /**
   * procedure的状态：
   * CREATE_TABLE_PRE_OPERATION 协处理pre方法
   * CREATE_TABLE_WRITE_FS_LAYOUT  表信息持久化到HDFS，同时创建region的目录
   * CREATE_TABLE_ADD_TO_META         添加region的元数据信息到meta表
   * CREATE_TABLE_ASSIGN_REGIONS       分配region
   * CREATE_TABLE_UPDATE_DESC_CACHE    更新TableDescriptor缓存
   * CREATE_TABLE_POST_OPERATION        协处理 post 方法
   * NO_MORE_STATE/HAS_MORE_STATE
   *
   */
  @Override
    ...
    try {
      switch (state) {
        ...
        case CREATE_TABLE_WRITE_FS_LAYOUT:
          /**
           * 确保将HDFS上面关于这张表（以前可能存在过）的所有数据删除干净
           */
