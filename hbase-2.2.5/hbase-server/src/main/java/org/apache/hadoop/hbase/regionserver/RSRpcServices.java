...
public class RSRpcServices implements HBaseRPCErrorHandler,
  ...
  RSRpcServices(HRegionServer rs, LogDelegate ld) throws IOException {
    ...
    try {
      /**
       * Master 或者 RegionServer 是怎么处理客户端请求的:
       *        1. 基本组件：ProcedureExecutor + Scheduler
       *        2. 每次请求封装为Procedure，提交到一个队列，然后ProcedureExecutor从队列中按照Scheduler指定的策略对请求进行处理
       *
       * rpcSchedulerFactory 调度器工厂，具体实现是SimpleRpcSchedulerFactory（在多个处理器上做负载均衡）
       */
